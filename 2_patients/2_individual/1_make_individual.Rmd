...
```{r combine_datasets, class.source = "fold-hide", time_it = TRUE}
# gz or h5 ?
extension = stringr::str_split(string = list.files(count_matrix_dir),
                               pattern = "\\.") %>%
  lapply(., FUN = rev) %>%
  lapply(., `[[`, 1) %>%
  unlist() %>%
...
if (extension == "h5") {
  # Load matrices
  all_files = list.files(count_matrix_dir)
  names(all_files) = stringr::str_split(all_files, pattern = "_") %>%
    lapply(., `[[`, 1) %>% unlist() # GSM...
  
  mat_list = lapply(names(all_files), FUN = function(one_file_name) {
    one_file = all_files[one_file_name]
    mat = Seurat::Read10X_h5(paste0(count_matrix_dir, "/", one_file))
    colnames(mat) = paste0(one_file_name, "_", colnames(mat))
    return(mat)
  })
  names(mat_list) = names(all_files)
  rm(all_files)
  
  # Common genes and common gene order
  common_genes = lapply(mat_list, FUN = rownames) %>%
    Reduce(x = ., f = intersect)
  
  mat_list = lapply(mat_list, FUN = function(one_mat) {
    one_mat[common_genes, ]
  })
  rm(common_genes)
  
  # Common matrix
  common_matrix = Reduce(x = mat_list, f = cbind)
  rm(mat_list)
  
  # Seurat object
  sobj = Seurat::CreateSeuratObject(counts = common_matrix,
                                    project = sample_name)
  sobj$log_nCount_RNA = log(sobj$nCount_RNA)
  
  rm(common_matrix)
} else {
  # gz : load count matrix and make Seurat object
  sobj = aquarius::load_sc_data(count_matrix_dir,
